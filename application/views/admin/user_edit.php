<section class="users_details">
	<div class="container">
		...
		<h2 class="main_head">Edit User <span><a href="<?php echo base_url().'user' ?>" class="ct_btn">Back to all users</a></span></h2>
		<?php// print_r($result['first_name']);die('vvv'); ?>
		<form class="row" id="up_form" method="post" action="<?php echo base_url().'user/edit/'.$result['id'] ?>">
	      <!--- Success Message --->
	      <?php if ($this->session->flashdata('up_success')) { ?>
	      <div class="alert alert-success" role="alert"><?php echo $this->session->flashdata('up_success'); ?></div>
	      <?php }?>

	      <!---- Error Message ---->
	      <?php if ($this->session->flashdata('up_error')) { ?>
	      <div class="alert alert-danger" role="alert"><?php echo $this->session->flashdata('up_error'); ?></div>
	      <?php } ?>
